...
\begin{document}
	...
	\\
	The intent of this document is not \textit{thesis writing}, but is instead meant as an update to work currently happening that will later be incorporated into the thesis
...
%	\end{itemize}
	...
	The current simulation uses a simple thin-airfoil theory model of a plane consisting of a wing, stabilizer, and propulsor shown in \cref{fig:TAT-model}. 
	The control vector representation for this system is $\mathbf{u} = \begin{Bmatrix} \delta_f,\; \delta_e,\; \delta_T \end{Bmatrix}$. 
	Lift and drag coefficients are calculated using thin-airfoil theory and the aircraft geometry; drag is calculated using a simple constant + quadratic term. 
	The throttle forces are calculated using a simple proportional model. 
	  \\
	\begin{figure}
		\centering
	...
	The force summation for wing, tail, and throttle contributions at the point mass are shown in \cref{eq:TAT-lift,eq:TAT-moment,eq:TAT-drag} in the aerodynamic coordinate system. 
	The summation includes the terms from the offset from $\ell_T \cdot T$ and the tail volume $\ell_ \cdot L_\mathrm{tail}$. 
	Forces are converted back into the body coordinate system using \cref{eq:aero-to-body}.
	
	 
	\begin{align}
		L_\mathrm{tot} &= L_\mathrm{wing} + L_\mathrm{tail} \label{eq:TAT-lift} \\
		M_\mathrm{tot} &= M_\mathrm{wing} + M_\mathrm{tail} + T \ell_t - L_\mathrm{tail} \ell_e \label{eq:TAT-moment} \\ 
		X_\mathrm{tot} &= D_\mathrm{wing} + D_\mathrm{tail} - T \label{eq:TAT-drag}
	\end{align}
	...
	\begin{equation} \label{eq:aero-to-body}
		T_b^e = \begin{bmatrix}
			\cos{\alpha} & \sin{\alpha} \\
			-\sin{\alpha} & \cos{\alpha}
		\end{bmatrix}
	...
	The velocity fields in this example 
